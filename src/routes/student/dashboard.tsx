...
import { AlertTriangle, BookOpen, CheckCircle2, Clock, GraduationCap, ShieldAlert, User } from "lucide-react";
...
function StudentDashboardContent() {
  ...
  const myPass = permissionByStudent[rollNo];

  const myViolations = reports.filter((r) => r.studentId === rollNo);
  const pendingExplanation = myViolations.find((r) => !r.explanation);
  // Count ONLY confirmed current semester violations (exonerated cases excluded)
  const confirmedViolations = myViolations.filter((r) => r.status !== "resolved").length;
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-start gap-3.5">
            ...
            <div>
              ...
              <h3 className="text-sm font-bold text-foreground mt-0.5">
                Violation Case {pendingExplanation.id} — {pendingExplanation.className}
              </h3>
              <p className="text-xs text-muted-foreground mt-1">
                A violation report was logged for {pendingExplanation.className} at {pendingExplanation.incidentTime}. Submit your explanation before HOD review.
              </p>
            </div>
          </div>
          <Button asChild className="rounded-xl font-bold bg-amber-600 hover:bg-amber-700 text-white shrink-0 h-10 px-5 shadow-xs">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="card-surface p-5 rounded-2xl border border-border shadow-2xs">
          <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider block">Active Gate Pass</span>
          <p className="text-base font-bold text-foreground mt-2">
            {myPass ? myPass.reason : "No Active Gate Pass"}
          </p>
          <p className="text-[11px] text-muted-foreground mt-1">
            {myPass ? `Valid until ${myPass.validUntil}` : "In class session"}
          </p>
        </div>

        <div className="card-surface p-5 rounded-2xl border border-border shadow-2xs">
          <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider block">Current Semester Violations</span>
          <p className="text-3xl font-extrabold text-foreground mt-2">{confirmedViolations}</p>
          <p className="text-[11px] text-muted-foreground mt-1">Confirmed current-semester count</p>
        </div>

        <div className="card-surface p-5 rounded-2xl border border-border shadow-2xs">
          <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider block">Academic Status</span>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div>
            ...
            <div className="mt-4 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">CSE &bull; 3rd Year &bull; Semester 6</p>
              </div>
            </div>
          </div>
        ...
        <section className="card-surface p-6 rounded-2xl border border-border shadow-xs">
          ...
          <div className="mt-4 space-y-2.5 text-xs">
            {[
              { time: "09:00 AM – 10:00 AM", subject: "Compiler Design", room: "Room C-201", active: false },
              { time: "10:00 AM – 11:00 AM", subject: "Data Structures", room: "Room C-204", active: true },
              { time: "11:15 AM – 12:15 PM", subject: "Database Management Systems", room: "Lab C-105", active: false },
            ].map((slot) => (
              <div
                key={slot.time}
